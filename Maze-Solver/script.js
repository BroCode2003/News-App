...
class TableCell {
  constructor(row, column) {
     ...
     *
     * Wenn Sie neue Aktionen (z. B. Diagonalpass) hinzufügen möchten, können Sie diese hier hinzufügen
     * Außerdem müssen Sie die Funktion „qTableInit“ und die Funktion „rTableGenerator“ aktualisieren
     * und „findPosibleActions“-Funktion
     *
     * Oder Sie können ein Objekt mit dem Namen „Richtung“ erstellen und es einfacher machen
     */
    this.north = 0;
    this.east = 0;
    this.south = 0;
...
function generateMap() {
  ...
  rowSize =
    document.querySelector("input#row").value != ""
      ? document.querySelector("input#row").value
      : defaultMapSize;
  columnSize =
    document.querySelector("input#column").value != ""
      ? document.querySelector("input#column").value
      : defaultMapSize;
...
async function addWallsRandomly(rowSize, columnSize) {
  ...
  let wallRate =
    rateInput.value == "" ? defaultWallRate : rateInput.value / 100;
...
async function train() {
  ...
  for (let episode = 0; episode < goalEpisode; episode++) {
    while (
      !walls.includes(currentPoint.div) &&
      currentPoint.div.id != "goalPoint"
    ) {
      if (!isTraining) return;
      ...
      if (currentPoint.div.id == "startPoint") break;
    }
    if (waitTime > 0) {
      window.requestAnimationFrame(() => {
